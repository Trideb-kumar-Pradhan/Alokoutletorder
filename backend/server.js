...
// Connect to MongoDB
mongoose.connect('mongodb+srv://tridebkumarp:1MVwgCTwBPjvMl5O@cluster0.kdhkovk.mongodb.net/outleorder', {
  useNewUrlParser: true,
  useUnifiedTopology: true
}).then(() => {
...
// Routes
app.post('/submit-order', async (req, res) => {
  const { empcode, name, address, email, cart } = req.body;
  ...
  try {
    ...
    // Create and save new order
    const newOrder = new Order({ empcode, name, address, email, cart });
    await newOrder.save();
